# 融合平台服务 运维手册（OPERATIONS）

> 服务：sm-fusion-platform（端口 8025）　版本：4.2.0

## 1. 服务概述与依赖

- 定位：SM 平台协作智能微服务，承接 融合平台 业务能力。
- 上游：API-Gateway。
- 下游依赖：PostgreSQL（数据库 sm_fusion_platform）、SM-Audit-Log-Center（审计）、SM-IAM（认证）、Vault（密钥）。
...
- 查 Pod：`kubectl get pods -n sm-prod -l app.kubernetes.io/name=sm-fusion-platform`。
