# sm-fusion-platform 多环境矩阵（MULTI_ENV_MATRIX）
...
| 数据库 | 共享开发库 sm_fusion_platform | 独立 staging 库 | 生产主库 + 只读副本 |
| 域名 | 无 Ingress（端口转发） | sm-fusion-platform.staging.sm.example.com | sm-fusion-platform.sm.example.com |
