# 融合平台服务 上线检查清单（DEPLOYMENT_CHECKLIST）

> 服务：sm-fusion-platform（端口 8025）
...
2. `helm upgrade --install sm-fusion-platform helm/sm-fusion-platform -n sm-prod -f values-prod.yaml`。
...
- 步骤：`helm rollback sm-fusion-platform <上一 revision>`，或 ArgoCD 回退。
- 验证：指标恢复 + 冒烟通过。
