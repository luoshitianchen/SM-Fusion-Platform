...
当前版本：`v4.2.0`。桌面端会检查 GitHub 最新正式版本，仅提示并打开发布页，不会静默下载或执行更新。
...
## v4.2 企业套件全量落地升级
- 服务矩阵 28 项全部落地真实业务实现：25 个模板项目替换为真实领域代码（数据模型、业务流转、统计接口与单元测试），核心仓库（Fusion/ERP/Knowledge Bot）保持真实实现。
- 安全加固：统一 fail-closed 内部鉴权（配置内部密钥后缺令牌写操作返回 401）；SM4-CBC + SM3-MAC 密文防篡改；KMS 信封加密消除密钥明文落库；API 网关改为白名单路由，杜绝路径穿越与限流绕过。
- 服务目录版本对齐：fusion 4.2.0、erp 2.5.0、knowledge 2.5.0，其余 25 项统一 2.0.0 并补齐真实能力描述。
- 全链路统一质量门：ruff + 覆盖率 ≥70% + compileall + docker compose 配置校验 + pip-audit/bandit/SBOM/gitleaks。
- 全部 28 仓库本地 ruff 与 pytest 全绿（合计 247 条用例）。
